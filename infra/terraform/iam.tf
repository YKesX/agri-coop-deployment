...
resource "aws_iam_role" "ec2_app" {
  ...
  tags = { Name = "${local.project_name}-ec2-role" }
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:ssm:eu-central-1:${data.aws_caller_identity.current.account_id}:parameter/${local.project_name}/*"
      }
    ]
  })
}
...
resource "aws_iam_role" "lambda" {
  ...
  tags = { Name = "${local.project_name}-lambda-role" }
}
